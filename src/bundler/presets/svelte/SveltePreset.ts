...
import { SvelteTransformer } from '../../transforms/svelte';
import { Preset } from '../Preset';

export class SveltePreset extends Preset {
  ...
  async init(bundler: Bundler): Promise<void> {
    ...
    await Promise.all([
      ...
      this.registerTransformer(new SvelteTransformer()),
  ...
  mapTransformers(module: Module): Array<[string, any]> {
    // Svelte components
    if (/\.svelte$/.test(module.filepath)) {
      return [
        ['svelte-transformer', {}],
        [
          'babel-transformer',
